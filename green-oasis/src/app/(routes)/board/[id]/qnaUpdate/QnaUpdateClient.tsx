...
export const QnaUpdateClient: React.FC<QnaUpdateProps> = ({qnaDetail}) => {
    ...
    const handleSave = async () => {
        try {
            ...
            if (!qnaId){
                Alert.error('수정 오류','Q&A ID가 없습니다');
                return;
            }
            const result = await qnaService.updateQna(flag, qnaId, insertData);
            if (result > 0) {
                Alert.success('저장 성공','저장 되었습니다.');
                router.push(`/board/${qnaId}/qnaDetail`);
                router.refresh();
            } else {
                Alert.error('저장 실패','저장 실패하였습니다.')
            }
        } catch (error) {
            console.error("QnaUpdateClient Error:", error);
            throw error;
        }
    };

    const handleCancle = useCallback(() => {
        router.push(`/board/${qnaDetail.QNA_ID}/qnaDetail`);
    }, [router])
                        ...
                        <div className="right">
                            <div>
                                <ol className="breadcrumb m-0">
                                    ...
                                    <li className="breadcrumb-item active">Q&A 수정</li>
                                </ol>
                            </div>
                        </div>
                    </div>
...
export default QnaUpdateClient;
